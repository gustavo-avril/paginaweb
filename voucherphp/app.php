...
preg_match('/NOMBRE:(.*)DNI/', $text, $nombre);
preg_match('/NOMBRE:(.*)PASAPORTE/', $text, $nombre2);
preg_match('/DNI (.*)FECHA DE NACIMIENTO/', $text, $dni);
preg_match('/PASAPORTE (.*)FECHA DE NACIMIENTO/', $text, $pasaporte);
preg_match('/FECHA DE NACIMIENTO: (.*?)PLAN/', $text, $dob);
preg_match('/PLAN: (.*)DESTINO/', $text, $planVoucher);
...
preg_match('/TEL.:(.*?)AGENCIA/', $text, $tlf);
preg_match('/AGENCIA:(.*)/', $text, $agencia);
preg_match('/ AL (.*?)FECHA DE EMISION/', $text, $to);
...
if($plan === 'inf-15' && $idioma === 'en'){
  ...
  $mpdf->WriteHTML($stylesheet,1);	
  $mpdf->WriteHTML($template);
  $mpdf->Output($idioma . '-' . $filename, D, D); 
  echo $html;
